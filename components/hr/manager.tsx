        ...
        <table className="w-full min-w-[720px] border-collapse text-left font-mono text-[12px]">
          <thead>
            <tr className="border-b border-fg/15 text-fg/45">
              {["Short Link", "Destination", "Created", "Actions"].map((h) => (
          ...
          <tbody>
            ...
            {!loading && links.length === 0 && (
              <tr>
                <td className="px-4 py-8 text-center uppercase tracking-[0.1em] text-fg/40" colSpan={4}>
                  No links yet.
                </td>
              </tr>
            )}
            ...
            {links.map((link) => (
              <tr key={link.shortId} className="border-b border-fg/8 text-fg/80 transition-colors hover:bg-fg/[0.03]">
                ...
                <td className="space-x-3 whitespace-nowrap px-4 py-3">
                  {editingId === link.shortId ? (
                    <>
                      <button type="button" onClick={() => handleUpdate(link.shortId)} className={`${linkAction} text-fg hover:text-sky`}>
                        Save
                      </button>
                      <button type="button" onClick={cancelEditing} className={`${linkAction} text-fg/50 hover:text-fg`}>
                        Cancel
                      </button>
                    </>
                  ) : (
                    <>
                      <button type="button" onClick={() => handleCopy(link.shortUrl)} className={`${linkAction} text-fg hover:text-sky`}>
                        Copy
                      </button>
                      <button type="button" onClick={() => startEditing(link)} className={`${linkAction} text-fg hover:text-sky`}>
                        Edit
                      </button>
                      <button type="button" onClick={() => handleDelete(link.shortId)} className={`${linkAction} text-ember`}>
                        Delete
                      </button>
                    </>
                  )}
